            ...
            <div className="container mx-auto px-4">
                ...
                {showSetupPrompt && !hasCompleteProfile() && (
                    <motion.div
                        ...
                        className="mb-6 p-4 bg-gray-950/5 border border-black/10 flex items-center justify-between gap-4"
                    >
                        <div className="flex items-center gap-3">
                        ...
                        <Link
                            ...
                            className="px-4 py-2 bg-gray-950 text-white text-sm tracking-wider hover:bg-gray-800 transition flex-shrink-0"
                        >
                            SET UP
                        </Link>
                    </motion.div>
                                ...
                                <Link href={`/shop/${match.product.id}`} className="block">
                                    <div className="relative aspect-[3/4] mb-3 overflow-hidden bg-gray-100">
                                        ...
                                        {match.isPerfectMatch && (
                                            <div className="absolute top-3 right-3 bg-gray-950 text-white px-2 py-1 text-[10px] font-bold flex items-center gap-1">
                                        ...
                                        <button type="button" onClick={(e) => {
                                            ...
                                            className="absolute bottom-3 left-3 right-3 bg-white text-black py-2.5 text-xs tracking-wider font-medium opacity-0 translate-y-3 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 hover:bg-gray-950 hover:text-white text-center"
                                        >
                                            QUICK ADD
                                        </button>
                                    </div>
                                ...
                                {match.matchReasons.length > 0 && (
                                    <div className="flex flex-wrap gap-1">
                                        {match.matchReasons.slice(0, 2).map((reason) => (
                                            ...
                                            >
                                                <Check className="size-2.5 text-green-500" weight="bold" />
                                                {reason}
                                            </span>
                                        ))}
                                    </div>
