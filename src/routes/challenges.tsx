    ...
    return (
      <div className="bg-white border border-gray-200 p-6">
        <div className="text-center">
          ...
          <Link href="/login" className="inline-block px-4 py-2 bg-gray-950 text-white text-sm">
            SIGN IN
          </Link>
        </div>
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {stats.badges.map((badge) => {
              ...
              return (
                ...
                >
                  <info.icon className="size-3" weight="fill" />
                  {info.label}
                </motion.div>
              );
            })}
      ...
      <div className="divide-y divide-gray-100 max-h-[400px] overflow-y-auto">
        {leaderboard.slice(0, 10).map(({ submission, rank }) => {
          ...
          return (
            ...
            >
              {/* Rank */}
              <div className={`size-8 flex items-center justify-center font-bold text-sm ${rank === 1 ? "bg-yellow-500 text-white" :
              ...
              <div className="flex items-center gap-1 text-sm text-gray-500">
                <Heart className="size-4" weight={submission.votes > 0 ? "fill" : "regular"} />
                {submission.votes}
              </div>
            </motion.div>
          );
    ...
    >
      <div className="relative aspect-[3/4]">
        ...
        {rankBadge && (
          ...
          >
            <rankBadge.icon className="size-3" weight="fill" />
            {rankBadge.label}
          </motion.div>
        )}

        {isOwnSubmission && (
          <div className="absolute top-3 right-3 bg-gray-950 text-white px-2 py-1 text-xs">
            YOUR ENTRY
          </div>
        )}
        ...
        <AnimatePresence>
          {showVoteAnimation && (
            <motion.div
              ...
              className="absolute inset-0 flex items-center justify-center bg-gray-950/20"
            >
              <Heart className="size-16 text-red-500" weight="fill" />
            </motion.div>
          )}
        </AnimatePresence>

        <div className="absolute inset-0 bg-gradient-to-t from-gray-950/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition" />
      </div>
      ...
      <div className="p-4">
        <div className="flex items-center gap-3 mb-3">
          {submission.userAvatar ? (
            ...
            <div className="size-8 bg-gray-200 rounded-full flex items-center justify-center text-sm font-medium">
          ...
          >
            <Heart
              className={`size-4 transition-colors ${hasVoted ? "text-red-500" : "text-gray-400"}`}
    ...
    ><button type="button" aria-label="Close" className="absolute inset-0 w-full h-full cursor-default" onClick={onClose} />
      ...
      >
        <div className="p-6 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white z-10">
          ...
          <button aria-label="x" type="button" onClick={onClose} className="p-2 hover:bg-gray-100 transition rounded-full">
            <X className="size-5" />
          </button>
        </div>
              ...
              <div key={s.num} className="flex items-center gap-2 flex-1">
                <div className="flex flex-col items-center">
                  <motion.div
                    ...
                    className={`size-10 flex items-center justify-center text-sm font-medium rounded-full ${s.num <= step ? "text-white" : "text-gray-400"
                      }`}
                  >
                    {s.num < step ? <CheckCircle className="size-5" weight="bold" /> : s.num}
                  </motion.div>
                ...
                {i < 2 && (
                  <div className={`flex-1 h-0.5 ${s.num < step ? "bg-gray-950" : "bg-gray-200"}`} />
                )}
              </div>
            ))}
          </div>
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              ...
              >
                ...
                {!currentPreview ? (
                  <>
                    ...
                    >
                      <input
                        ...
                        type="file"
                        accept="image/*"
                        aria-label="Upload challenge photo"
                      ...
                      <div className="relative z-10">
                        <motion.div
                          ...
                          className="size-16 bg-gray-100 rounded-full flex items-center justify-center mx-auto mb-4"
                        >
                          <Camera className="size-8 text-gray-400" weight="light" />
                        </motion.div>
                      ...
                      {showUrlInput && (
                        ...
                        >
                          <div className="flex gap-2">
                            <input
                              type="url"
                              aria-label="Challenge image URL"
                              value={imageUrl}
                            ...
                            />
                            {imageUrl && (
                              <button aria-label="x" type="button" onClick={() => setImageUrl("")}
                  ...
                  >
                    <div className="relative aspect-[3/4] max-w-xs mx-auto bg-gray-100 rounded-xl overflow-hidden shadow-lg">
                      <Image src={currentPreview} alt="Preview" fill className="object-cover"  sizes="(max-width: 768px) 100vw, 50vw" />

                      {/* Overlay with actions */}
                      <div className="absolute inset-0 bg-gradient-to-t from-gray-950/50 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity">
                        <div className="absolute bottom-4 left-4 right-4 flex justify-center gap-2">
                          <button aria-label="camera" type="button" onClick={() => fileInputRef.current?.click()}
                          ...
                          <button aria-label="trash" type="button" onClick={clearImage}
                      ...
                      {/* Quick delete button */}
                      <button aria-label="x" type="button" onClick={clearImage}
                        className="absolute top-3 right-3 size-8 bg-gray-950/50 hover:bg-gray-950/70 rounded-full flex items-center justify-center text-white transition">
                        <X className="size-4" />
                      </button>
                ...
                <button type="button" onClick={() => setStep(2)}
                  ...
                  className="w-full bg-gray-950 text-white py-4 text-sm tracking-wider font-medium hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition rounded-lg flex items-center justify-center gap-2"
                >
                  CONTINUE
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto">
                  {products.map((product) => (
                    ...
                    >
                      <div className="relative size-12 bg-gray-100 flex-shrink-0">
                        {product.images?.[0] && (
                        ...
                        {selectedProducts.includes(product.id) && (
                          <div className="absolute inset-0 bg-gray-950/50 flex items-center justify-center">
                            <CheckCircle className="size-5 text-white" weight="bold" />
                          </div>
                        )}
                      </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={() => setStep(3)}
                    ...
                    className="flex-1 bg-gray-950 text-white py-3 text-sm tracking-wider hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition"
                  >
                    CONTINUE
                  </button>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-4">
                ...
                <textarea aria-label="Describe your look..."
                  value={caption}
                ...
                <div className="flex gap-3">
                  ...
                  <button type="button" onClick={handleSubmit}
                    ...
                    className="flex-1 bg-gray-950 text-white py-3 text-sm tracking-wider hover:bg-gray-800 disabled:opacity-50 transition flex items-center justify-center gap-2">
                    {submitting ? (
                      <>
                        <SpinnerGap className="size-4 animate-spin" />
...
function ChallengeDetail({
  ...
  const sortedSubmissions = sortBy === "votes"
    ? topSubmissions
    : challenge.submissions.toSorted((a, b) => b.createdAt - a.createdAt);
  ...
  return (
    <div>
      {/* Back Button */}
      <button aria-label="caret left" type="button" onClick={onBack}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setSortBy("votes")}
                className={`px-3 py-1 text-sm ${sortBy === "votes" ? "bg-gray-950 text-white" : "bg-gray-100"}`}
              >
                Top Voted
              </button>
              <button type="button" onClick={() => setSortBy("recent")}
                className={`px-3 py-1 text-sm ${sortBy === "recent" ? "bg-gray-950 text-white" : "bg-gray-100"}`}
              >
                Recent
              </button>
            </div>
            ...
            <div className="text-center py-16 bg-gray-50 border border-gray-200">
              ...
              {canSubmit && (
                <button type="button" onClick={() => setShowSubmitModal(true)}
                  className="bg-gray-950 text-white px-6 py-3 text-sm tracking-wider hover:bg-gray-800 transition"
                >
                  SUBMIT YOUR ENTRY
                </button>
              )}
            ...
            <div className="space-y-3">
              {challenge.prizes.map((prize) => (
                <div key={prize.place} className="flex items-center gap-3">
                  <div className={`size-8 flex items-center justify-center text-sm font-bold ${prize.place === 1 ? "bg-yellow-100 text-yellow-700" :
  ...
  return (
    ...
    >
      <div className="relative aspect-[16/9]">
        ...
        {challenge.featured && (
          <div className="absolute top-3 right-3 bg-gray-950 text-white px-2 py-1 text-xs flex items-center gap-1">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="flex items-center gap-1 font-medium group-hover:gap-2 transition-all">
            View <CaretRight className="size-4" />
          </span>
        </div>
      </div>
    </motion.button>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            {[
              ...
              <div key={item.step} className="text-center">
                <div className="size-12 bg-gray-950 text-white flex items-center justify-center mx-auto mb-4">
...
export const Route = createFileRoute("/challenges")({ component: StyleChallengesPage });
