      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid md:grid-cols-2 gap-12 lg:gap-20">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative aspect-[3/4] bg-gray-100">
            <Image src={getCurrentImage()} alt={product.name} fill className="object-cover"  sizes="(max-width: 768px) 100vw, 50vw" />
            <button type="button" onClick={handleToggleWishlist} 
              className={`absolute top-4 right-4 size-12 flex items-center justify-center transition shadow-lg ${
            ...
            <button type="button" onClick={handleToggleCompare} 
              className={`absolute top-4 right-20 size-12 flex items-center justify-center transition shadow-lg ${
                inCompare ? "bg-gray-950 text-white" : "bg-white hover:bg-gray-50"
              }`}>
              <Scales className="size-5" />
            </button>
            
            {/* Virtual Try-On Floating Button */}
            <button aria-label="magic wand" type="button" onClick={() => setIsTryOnOpen(true)}
              className="absolute bottom-4 right-4 bg-gray-950 text-white px-4 py-3 flex items-center gap-2 shadow-xl hover:shadow-2xl hover:bg-gray-800 transition-all rounded-full group"
            >
              ...
              <div className="absolute bottom-4 left-4 flex gap-2">
                {product.colors.map((color) => (
                  <button type="button" key={color.name}
                    ...
                    className={`size-12 border-2 bg-white overflow-hidden transition-all ${
          ...
          >
              ...
              {hasDiscount ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="bg-amber-100 text-amber-700 px-2 py-1 text-xs font-medium flex items-center gap-1">
                    <TrendUp className="size-3" /> HIGH DEMAND
                  </span>
                </div>
              ) : (
                ...
                <div className="flex gap-3 flex-wrap">
                  {product.colors.map((color) => (
                    <button type="button" key={color.name}
                      onClick={() => handleColorSelect(color.name)}
                      title={color.name}
                      className={`group relative size-10 rounded-full border-2 transition-all ${
                        selectedColor === color.name
            ...
            <div className="mb-8">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-4">
                  <button aria-label="sparkle" type="button" onClick={() => setIsSizeModalOpen(true)} 
                    className="text-xs tracking-wider text-black flex items-center gap-1 hover:underline underline-offset-4"
                  >
                    <Sparkle className="size-3" /> Find My Size
                  </button>
              ...
              {sizeRecommendation && (
                <div className="mb-4 p-3 bg-green-50 border border-green-200 flex items-center gap-3">
                  <Sparkle className="size-4 text-green-600" />
              ...
              <div className="flex gap-3 flex-wrap">
                {sizes.map((size) => (
                  <button type="button" key={size} 
                    ...
                    className={`size-14 border text-sm tracking-wider transition-all relative ${
                      selectedSize === size 
            ...
            <div className="space-y-3 mb-10">
              {isOutOfStock ? (
                <button aria-label="bell" type="button" onClick={() => setIsStockNotifyModalOpen(true)} 
                  className="w-full py-4 text-sm tracking-wider font-medium transition flex items-center justify-center gap-2 bg-gray-900 text-white hover:bg-gray-950"
                >
                  <Bell className="size-4" /> NOTIFY ME WHEN AVAILABLE
                </button>
              ) : (
                <button type="button" onClick={handleAddToCart} 
                  className={`w-full py-4 text-sm tracking-wider font-medium transition flex items-center justify-center gap-2 ${
                    addedToCart ? "bg-green-600 text-white" : "bg-gray-950 text-white hover:bg-gray-900"
                  }`}>
                  {addedToCart ? <><Check className="size-4" /> ADDED TO BAG</> : "ADD TO BAG"}
                </button>
              )}
              
              {/* Virtual Try-On CTA - Prominent */}
              <button aria-label="magic wand" type="button" onClick={() => setIsTryOnOpen(true)} 
                className="w-full py-4 text-sm tracking-wider font-medium transition flex items-center justify-center gap-3 bg-gray-900 text-white hover:bg-gray-950 hover:shadow-xl hover:scale-[1.01] active:scale-[0.99] relative overflow-hidden group"
              >
                <MagicWand className="size-5" />
                ...
                <button type="button" onClick={handleToggleWishlist} 
                  className={`border py-4 text-sm tracking-wider font-medium transition flex items-center justify-center gap-2 ${
                    inWishlist 
                      ...
                      : "border-black hover:bg-gray-950 hover:text-white"
                  }`}>
                ...
                <div className="relative">
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => {
                      ...
                      className="size-10 flex items-center justify-center border border-gray-200 hover:border-black hover:bg-gray-950 hover:text-white transition"
                      title="Share on X"
                    >
                    ...
                    >
                      <PinterestLogo className="size-4" />
                    </button>
                    <button type="button" onClick={() => {
                        const url = typeof window !== "undefined" ? window.location.href : "";
...
export const Route = createFileRoute("/shop/$id")({ component: ProductPage });
