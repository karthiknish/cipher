  ...
  useEffect(() => {
    if (username) {
      ...
      if (inf) {
        ...
        const curated = inf.curatedProducts.flatMap((id) => {
        ...
        const featured = inf.featuredProducts.flatMap((id) => {
          const product = products.find((p) => p.id === id);
          return product ? [product] : [];
        }) as Product[];
  ...
  if (!influencer) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center">
        <SpinnerGap className="size-8 animate-spin text-gray-400 mb-4" />
  ...
  if (!influencer.isActive) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-4">
        ...
        <Link href="/shop" className="mt-6 px-6 py-3 bg-gray-950 text-white text-sm hover:bg-gray-800 transition">
          Browse Shop
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      ...
      <AnimatePresence>
        {showLiveStream && influencer.isLive && influencer.liveStreamUrl && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-gray-950/90 z-50 flex flex-col"
          >
            <div className="p-4 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="size-10 rounded-full bg-gradient-to-r from-red-500 to-pink-500 flex items-center justify-center">
                  <Broadcast className="size-5 text-white" weight="fill" />
                </div>
                <div>
              ...
              <button aria-label="x" type="button" onClick={() => setShowLiveStream(false)}
            ...
            <div className="flex-1 flex flex-col lg:flex-row">
              ...
              <div className="flex-1 relative">
                <iframe
                  ...
                  title={`${influencer.displayName} live stream`}
      ...
      <div className="relative h-48 md:h-64 bg-gradient-to-r from-gray-900 to-gray-800">
        ...
        {influencer.isLive && (
          ...
          >
            <Broadcast className="size-4 animate-pulse" weight="fill" />
      ...
      <div className="relative px-6 md:px-12 pb-8">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative -mt-16 mb-4">
            <div className="size-32 rounded-full border-4 border-white overflow-hidden bg-gray-200">
              {influencer.avatar ? (
                <Image
                ...
                <div className="size-full bg-gradient-to-br from-gray-300 to-gray-400 flex items-center justify-center text-white text-3xl font-bold">
            ...
            {influencer.isVerified && (
              <div className="absolute bottom-0 right-0 size-8 bg-blue-500 rounded-full flex items-center justify-center border-2 border-white">
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            ...
            {/* Actions */}
            <div className="flex items-center gap-3">
              <button aria-label="check circle" type="button" onClick={handleShare}
                className="flex items-center gap-2 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-full text-sm transition">
                {copied ? <CheckCircle className="size-4 text-green-600" /> : <Share className="size-4" />}
                {copied ? "Copied!" : "Share"}
              </button>
              {influencer.isLive && (
                <button aria-label="play" type="button" onClick={() => setShowLiveStream(true)}
      ...
      <div className="border-t border-gray-100">
        <div className="max-w-5xl mx-auto px-6 md:px-12 py-8">
          ...
          <div className="flex items-center gap-4 mb-8">
            <button type="button" onClick={() => setActiveTab("featured")}
              className={`px-4 py-2 text-sm font-medium transition ${
                activeTab === "featured" ? "bg-gray-950 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}
            >
              Featured Picks
            </button>
            <button type="button" onClick={() => setActiveTab("all")}
              className={`px-4 py-2 text-sm font-medium transition ${
                activeTab === "all" ? "bg-gray-950 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}
            >
                ...
                {featuredProducts.map((product, i) => (
                  ...
                  >
                    <Link href={`/shop/${product.id}?ref=${influencer.username}`}>
                      <div className="relative aspect-[4/5] mb-3 overflow-hidden bg-gray-100">
                        <Image
                         ...
                         sizes="(max-width: 768px) 100vw, 50vw" />
                        <div className="absolute top-2 left-2 px-2 py-1 bg-gray-950 text-white text-xs">
                          TOP PICK
                        </div>
                      </div>
                    </Link>
                    ...
                    <div className="absolute top-2 right-2 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button aria-label="shopping bag" type="button" onClick={() => handleAddToCart(product)}
                        className="p-2 bg-white rounded-full shadow-lg hover:scale-110 transition-transform"
                      >
      ...
      {/* CTA Banner */}
      <div className="bg-gray-950 text-white py-12 px-6">
...
export const Route = createFileRoute("/shop/creator/$username")({ component: CreatorStorefront });
