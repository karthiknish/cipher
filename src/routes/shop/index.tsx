      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12 pb-6 border-b border-gray-200">
          <div className="flex items-center gap-4">
            <button aria-label="x" type="button" onClick={() => setFilterOpen(!filterOpen)}
              className="flex items-center gap-2 text-sm tracking-wider font-medium hover:opacity-70 transition"
            >
              {filterOpen ? <X className="size-4" /> : <Plus className="size-4" />}
              FILTER
            </button>
                ...
                <Link href={`/shop/${product.id}`} className="block">
                  <div className="relative aspect-[3/4] mb-4 overflow-hidden bg-gray-100">
                    ...
                    <button type="button" onClick={(e) => {
                      ...
                      className="absolute bottom-4 left-4 right-4 bg-white text-black py-3 text-sm tracking-wider font-medium opacity-100 translate-y-0 sm:opacity-0 sm:translate-y-4 sm:group-hover:opacity-100 sm:group-hover:translate-y-0 transition-all duration-300 hover:bg-gray-950 hover:text-white text-center"
