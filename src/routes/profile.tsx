...
import { LoyaltySection, StyleQuizSection, AddressFormModal } from "@/components/profile-sections";
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <label className="absolute -bottom-1 -right-1 size-8 bg-gray-950 text-white rounded-full flex items-center justify-center cursor-pointer hover:bg-gray-800 transition">
                <Camera className="size-4" />
                <input id="page-field-35" 
                  type="file" 
                  accept="image/*"
        ...
        <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Link href="/orders" className="group p-6 border border-gray-200 hover:border-black transition">
                  <Package className="size-6 mb-4 text-gray-400 group-hover:text-black transition" />
                  <h4 className="font-medium mb-1">Orders</h4>
                  <p className="text-sm text-gray-500">View order history</p>
                  <CaretRight className="size-5 mt-4 text-gray-300 group-hover:text-black transition" />
                </Link>

                <Link href="/wishlist" className="group p-6 border border-gray-200 hover:border-black transition">
                  <Heart className="size-6 mb-4 text-gray-400 group-hover:text-black transition" />
                  <h4 className="font-medium mb-1">Wishlist</h4>
                  <p className="text-sm text-gray-500">Saved items</p>
                  <CaretRight className="size-5 mt-4 text-gray-300 group-hover:text-black transition" />
                </Link>

                <button aria-label="gift" type="button" onClick={() => setActiveTab("rewards")}
                  className="group p-6 border border-gray-200 hover:border-black transition text-left"
                >
                  <Gift className="size-6 mb-4 text-gray-400 group-hover:text-black transition" />
              ...
              <div className="pt-8 border-t border-gray-200">
                <button aria-label="sign out" type="button" onClick={handleSignOut}
            ...
            >
              ...
              {userAchievements && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                  ...
                  <div className="bg-gray-50 p-4 text-center">
                    <User className="size-6 mx-auto mb-2 text-gray-600" />
              ...
              <div>
                <h3 className="font-medium mb-4 flex items-center gap-2">
                  <Medal className="size-5" /> Unlocked Badges ({getUserBadges().length})
                </h3>
                {badgesLoading ? (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {getUserBadges().map((badge) => (
                      <div
                        key={badge.id}
                        role={badge.isNew ? "button" : undefined}
                        tabIndex={badge.isNew ? 0 : undefined}
                        onClick={() => badge.isNew && markBadgeAsSeen(badge.id)}
                        onKeyDown={(e) => {
                          if (badge.isNew && (e.key === "Enter" || e.key === " ")) {
                            e.preventDefault();
                  ...
                  <div className="text-center py-12 bg-gray-50">
                    <Medal className="size-12 mx-auto mb-4 text-gray-300" />
              ...
              <div>
                <h3 className="font-medium mb-4 flex items-center gap-2">
                  <Trophy className="size-5" /> Available Badges ({getLockedBadges().length})
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {activeTab === "addresses" && (
            ...
            >
              ...
              {profile?.savedAddresses && profile.savedAddresses.length > 0 ? (
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          {address.isDefault && (
                            <span className="px-2 py-0.5 bg-gray-950 text-white text-xs">Default</span>
                          )}
                        </div>
                        <div className="flex gap-2">
                          <button aria-label="pencil simple" type="button" onClick={() => handleEditAddress(address)}
                            className="p-2 hover:bg-gray-100 transition"
                          >
                            <PencilSimple className="size-4" />
                          </button>
                          <button aria-label="trash" type="button" onClick={() => handleDeleteAddress(address.id)}
                ...
                <div className="text-center py-12 bg-gray-50">
                  <MapPin className="size-12 mx-auto mb-4 text-gray-300" />
                  <p className="text-gray-500 mb-4">No saved addresses yet</p>
                  <button type="button" onClick={() => setIsAddressModalOpen(true)}
                    className="bg-gray-950 text-white px-6 py-3 text-sm tracking-wider hover:bg-gray-900 transition"
                  >
                    ADD ADDRESS
                  </button>
                </div>
          ...
          {activeTab === "preferences" && (
            ...
            >
              ...
              <div>
                <h3 className="font-medium mb-4 flex items-center gap-2">
                  <Palette className="size-4" /> Favorite Colors
                </h3>
              ...
              <div>
                <h3 className="font-medium mb-4 flex items-center gap-2">
                  <Ruler className="size-4" /> Preferred Fit
                </h3>
                <div className="flex flex-wrap gap-3">
                  {fits.map((fit) => (
              ...
              <button aria-label="floppy disk" type="button" onClick={handleSavePreferences}
                disabled={isSaving}
                className="bg-gray-950 text-white px-8 py-4 text-sm tracking-wider font-medium hover:bg-gray-900 transition disabled:opacity-50 flex items-center gap-2">
